...
def get_blog_list_common_data(request, blogs_all_list):
    paginator = Paginator(blogs_all_list, settings.NUM_OF_EACH_PAGE)  # 默每2页进行分页
...
def blog_detail(request, blog_pk):
    ...
    context = {}
    context['previous_blog'] = Blog.objects.filter(created_time__gt=blog.created_time).last()# 取最后一条
    context['next_blog'] = Blog.objects.filter(created_time__lt=blog.created_time).first()# 取第一条
    context['blog'] = blog
    # 响应
